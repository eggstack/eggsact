...
> `src/temporal/` (fixed-offset datetime + cron) is a separate `pub(crate)` leaf core, not part of `src/text/` — see [tools.md](tools.md) (temporal section) for its coverage.
