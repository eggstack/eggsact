...
- **Timeout lifecycle state machine**: `HANDLER_QUEUED (0) → HANDLER_RUNNING (1) → HANDLER_TIMEOUT_ACCOUNTING (2) → HANDLER_TIMED_OUT_ACCOUNTED (3) → HANDLER_FINISHED (4)`. Queued timeout increments only `total_timeouts`, NOT `timed_out_handlers`. Running timeout: `compare_exchange(HANDLER_RUNNING, HANDLER_TIMEOUT_ACCOUNTING)` → increment `timed_out_handlers` → `HANDLER_TIMED_OUT_ACCOUNTED`. Handler exit: `swap(HANDLER_FINISHED)` — only decrements `timed_out_handlers` if previous state was `HANDLER_TIMED_OUT_ACCOUNTED`. This guarantees exactly one increment and one decrement per running-timeout event, and zero for queued-timeout events.
