...
- **Never hand-edit generated docs** — README tool tables, architecture profile reference, and `generated/tool-cards.md` are generated by `cargo run --features dev-tools --bin generate-docs`. Edit `ToolSpec` entries in `src/mcp/specs/` instead.
...
- **Parity tests require `eggcalc`** Python package at `../eggcalc`. See `docs/parity.md` for 34 known failures (C1–C6). Do not treat these as regressions.
