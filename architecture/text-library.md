...
The `src/text/` module provides **26 text processing modules** plus auto-generated confusable data, used by MCP tools and available as a library API. It covers measurement, diffing, validation, transformation, regex processing, Unicode policy enforcement, shell parsing, path analysis, markdown inspection, patch operations, script classification, and more.
...
| `script` | `script.rs` | (authoritative, used by `unicode_tools`, `unicode_policy`, `identifier`) | `script_of()`, `policy_script_of()`, `is_legitimate_mixture()`, `is_script_bearing()` |
...
`script.rs` is the single authoritative source for script identity
(`script_of`, `policy_script_of`) and legitimate writing-system mixtures
(`is_legitimate_mixture`: Japanese Han/Hiragana/Katakana and Korean
Hangul/Han/Latin). `unicode_tools`, `unicode_policy`, and `identifier`
delegate to it instead of maintaining independent range tables.
...
- `confusables_generated.rs` is not a module — it is `include!`d by `confusables.rs` (`pub static CONFUSABLES: &[(u32, &str)]`), so the directory holds 27 files for 25 modules.
...
| `classify_hazard(c)` | Typed `UnicodeHazard` classification (bidi/join/invisible-format/variation-selector/combining-mark/control); the non-presentation source consumed by `text_measure`, `inspect_text_security`, and policy paths |
...
| `detect_mixed_scripts(text)` | Detect spoof script mixtures (legitimate Japanese/Korean mixtures excluded via `is_legitimate_mixture`), return script list and per-character positions |
...
| `confusable_skeleton(text)` | Whole-string UTS #39 skeleton for the pinned data epoch (NFD → mapping → NFD); two strings are confusable iff skeletons are exactly equal |
...
- **Rust**: Unicode XID rules via the `unicode-ident` crate (`XID_Start`/`XID_Continue` plus `_`), keyword detection kept separate; `is_valid_rust_identifier_ascii()` retains the explicit ASCII-only verdict for stricter policy surfaces
...
- **Collisions**: UTS #39 `confusable` pairs (exact whole-string skeleton equality, one skeleton per normalized identifier), `near_match` pairs (edit distance ≤ 1 without skeleton equality), casefold collisions, normalization collisions
