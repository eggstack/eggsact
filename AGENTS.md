...
src/
  ...
  text/             # text processing library (25 modules)
    regex_engine.rs # regex backend classifier (classify_pattern, compile_regex, CompiledRegex, CompileError, normalize_flags, RegexEngineUsed, RegexFeature)
...
- `architecture/calculator.md` — NL pipeline (31-step), AST evaluator, 140+ units, 50+ constants, EvalContext
...
- `architecture/machine-codes.md` — ~145 response codes, finding helpers, severity/disposition/verdict
...
- `docs/parity.md` — Python/Rust parity, 34 known failures (categories C1–C6)
...
- **Regex backend auto-selection**: `regex_finditer` and `validate_regex` use `compile_regex()` in `src/text/regex_engine.rs` to classify and compile patterns, selecting between Rust `regex` (fast, linear-time) and `fancy-regex` (lookaround/backreference support). The `CompiledRegex` enum ensures `engine_used` is always derived from the actual compiled backend. Outputs report `engine_used` (`"rust-regex"` or `"fancy-regex"`), `dialect` (`"eggsact-regex"`), `unsupported_features` for PCRE-only constructs, `execution_error` for runtime engine failures, and `policy_allowed` for safety-check results. Unsupported constructs return `REGEX_UNSUPPORTED_FEATURE` machine code. ASCII mode is not supported and returns `REGEX_ASCII_NOT_SUPPORTED`. This is NOT PCRE2.
- **Parity tests require `eggcalc`** Python package at `../eggcalc`. See `docs/parity.md` for the 34 known failures (categories C1–C6, accepted behavioral differences). Do not treat these as regressions.
...
- **`ensure_mcp_defaults()` has been removed** — MCP dispatch now creates `EvalContext::mcp_mode()` and sets it via `budget::with_eval_context()` thread-local bridge instead of calling `ensure_mcp_defaults()`/`set_mcp_mode()` globally.
...
- **Sync execution pool for budget-aware APIs**: `call_json_with_budget`, `call_json_with_context`, and `call_json_with_execution_context` route through `SyncExecutionPool` (8 workers, 32-slot queue) in `src/mcp/sync_pool.rs`. Queue saturation returns `RESOURCE_EXHAUSTED`. `call_json` remains direct (no pool, no elapsed-time enforcement). The MCP server path is NOT affected (uses Tokio `spawn_blocking`).
