...
The `src/text/` module provides **27 text processing modules** plus auto-generated confusable and Unicode-property data, used by MCP tools and available as a library API. It covers measurement, diffing, validation, transformation, regex processing, Unicode policy enforcement, shell parsing, path analysis, markdown inspection, patch operations, script classification, and more.
...
| `script` | `script.rs` | (authoritative diagnostics + resolved verdicts, used by `unicode_tools`, `unicode_policy`, `identifier`) | `script_of()`, `policy_script_of()`, `resolved_script_set()`, `is_mixed_script()`, `is_legitimate_mixture()` (legacy compat only), `is_script_bearing()` |
...
| `unicode_properties` | `unicode_properties.rs` + `unicode_properties_generated.rs` | (Unicode 18.0.0 UTS #39 / UAX #24 / UAX #9 tables; never hand-edit generated file) | `is_default_ignorable()`, `bidi_class_of()`, `bidi_mirror()`, `script_extensions()`, `augmented_script_set()`, `resolved_script_set()`, `is_mixed_script()`, `Unicode18BidiData` |
...
| `validate` | `validate.rs` | `validate_json`, `validate_regex`, `validate_brackets`, `validate_schema_light`, `json_shape`, `json_canonicalize`, `json_compare`, `json_extract`, `regex_finditer`, `regex_test` | `validate_json()`, `validate_regex()`, `validate_brackets()`, `validate_brackets_with_pairs()`, `validate_schema_light()`, `json_shape()`, `json_canonicalize()`, `json_compare()`, `json_extract()`, `regex_finditer()`, `regex_test()`, `list_dedupe()`, `list_sort()`, `python_style_canonical_string()` |
...
`script.rs` is the compatibility layer over `unicode_properties` for script identity:
diagnostic spellings (`script_of`, `policy_script_of`) are preserved while the
security verdict uses UTS #39 §5.1 resolved Script_Extensions sets
(`resolved_script_set`, `is_mixed_script` with Jpan/Kore/Hanb/Hntl augmentation
and Common/Inherited as ALL). Only `Zyyy` (Common) and `Zinh` (Inherited) are
ALL: `Zzzz` (Unknown, e.g. private-use/unassigned `Script_Extensions={Unknown}`)
is an ordinary singleton set that constrains the intersection — private-use
alone resolves to `{Zzzz}`, Common + private-use stays `{Zzzz}`, and Latin +
private-use is mixed. `is_legitimate_mixture` remains only for 1.x source
compatibility (Japanese Han/Hiragana/Katakana and Korean Hangul/Han/Latin
allowances); it does NOT implement the mixed-script predicate — notably,
Hangul+Han+Latin is mixed under resolved sets. `unicode_tools`, `unicode_policy`,
and `identifier` consume the resolved verdict instead of maintaining independent
range tables.

> `src/temporal/` (fixed-offset datetime + cron) is a separate `pub(crate)` leaf core, not part of `src/text/` — see [tools.md](tools.md) (temporal section) for its tool coverage and the [Temporal core](#temporal-core-srctemporal) section below for its contracts.

## Layering

`src/text/` is a leaf layer: its 27 modules depend only on each other (`crate::text::…`), third-party crates, and std. They must not depend on `crate::agent`, `crate::calc`, `crate::preflight`, `crate::services`, or `crate::tools`, and never call one tool handler from another (composition lives in `src/services/` typed composites and `src/tools/` JSON adapters).

Narrow exceptions to the rule:

- `text::diff` reads the cooperative-cancellation flag via the fully qualified `crate::mcp::budget::current_cancel_flag()` (no `use crate::mcp…` import). On cancellation it bails to a coarse result (`max(a_len, b_len)`) instead of failing.
- `temporal::cron` takes handler budgets from `crate::mcp::budget` and returns boxed `ToolResponse` errors on budget stop / calendar-range exhaustion. No tool-handler logic is involved.
- `text::synthesis` is documentation-only: its entire body is `pub mod detail` containing a doc-comment mapping table (Python `synthesis` functions → Rust sources). No executable code.
- `confusables_generated.rs` is not a module — it is `include!`d by `confusables.rs` (`pub static CONFUSABLES: &[(u32, &str)]`), and `unicode_properties_generated.rs` is a generated data module backing `unicode_properties.rs` (never hand-edit either), so the directory holds 30 files for 27 modules.
...
This engine is **not PCRE2**: `compile_regex()` (`src/text/regex_engine.rs`) auto-selects the `regex` vs `fancy-regex` backend via `classify_pattern()`, rejects unsupported PCRE constructs with `REGEX_UNSUPPORTED_FEATURE`, and every tool output reports which backend ran in `engine_used` (`"rust-regex"` or `"fancy-regex"`).
...
| `is_bidi_control(c)` | Typed bidi membership: exactly the 11 UAX #9 controls (LRM/RLM/LRE/RLE/PDF/LRO/RLO/LRI/RLI/FSI/PDI). Security decisions must use this, never `display.contains("BIDI")` |
| `is_join_control(c)` | Join controls (ZWNJ, ZWJ) |
| `has_security_invisible_hazard(c)` | Single security-verdict predicate for user-visible invisibles (Bidi/Join/InvisibleFormat/VariationSelector via `classify_hazard`); distinct from `Default_Ignorable_Code_Point` |
| `is_zero_width_char(c)` | Single source for zero-width policy findings (ZWSP/ZWNJ/ZWJ/WORD JOINER) |
| `classify_hazard(c)` | Typed `UnicodeHazard` classification (bidi/join/invisible-format/variation-selector/combining-mark/control); the non-presentation source consumed by `text_measure`, `inspect_text_security`, and policy paths. U+034F (CGJ, Mn) reports `InvisibleFormat`, not generic `CombiningMark` |
...
| `script_name(c)` | Script identity, delegated to the authoritative `script` module (Latin, Cyrillic, Greek, Hebrew, Arabic, Devanagari, Thai, Hiragana, Katakana, Han, Hangul, etc.) |
| `detect_mixed_scripts(text)` | UTS #39 §5.1 resolved-script verdict (`is_mixed_script`); observed per-character script list preserved for diagnostics, boolean derived from augmented Script_Extensions intersection (Japanese/Korean restriction allowances no longer decide) |
| `unicode_scripts(s)` | Return per-character script names |
| `confusables_count(s)` | Count characters that have confusable mappings |
| `reverse_confusables(ch)` | Source characters whose mapping is exactly the single target `ch` (multi-code-point sources excluded: component mapping is not equivalence) |
...
> 1.13.3), XID 18.0 (`unicode-ident` 1.0.26), UAX #9 algorithm
> (`unicode-bidi` 0.3.18, used WITHOUT its bundled Unicode 16 hardcoded data —
> always via the custom Unicode 18 `Unicode18BidiData` source), Script /
> Script_Extensions / Bidi_Class / mirroring / brackets 18.0.0 (generated
> `unicode_properties_generated.rs`). Never claim "all Unicode processing:
> 18.0.0"; the accurate statement is "confusables data: Unicode 18.0.0" plus
> "security skeleton/script/bidi properties: Unicode 18.0.0". Canonical
> (NFD/NFC) mappings are stability-guaranteed, so the skeleton relation is
> unaffected by the normalization epoch skew; see `architecture/generated-assets.md`.

| Function | Description |
|----------|-------------|
| `has_confusables(text)` | Returns `true` if any character has a confusable mapping (source mapping, not a collision verdict) |
| `find_confusables(text)` | Returns `Vec<(char, &'static str)>` — each confusable char and its target codepoint(s) (per-character source mappings) |
| `confusable_skeleton(text)` | Public UTS #39 `skeleton(X) = bidiSkeleton(LTR, X)` for the pinned data epoch (UAX #9 L1/L2 via version-correct tables → L3 mark fixup → L4 mirroring → internal skeleton); two strings are confusable iff skeletons are exactly equal |
| `internal_skeleton(text)` | UTS #39 internal stage (NFD → remove Default_Ignorable → mapping → NFD); additive helper for conformance testing |
| `bidi_skeleton_ltr(text)` | UTS #39 `bidiSkeleton(LTR, X)` with the spec fast path (no R/AL → internal); slow path runs UAX #9 L1/L2 paragraph-locally per paragraph (each paragraph substring through `unicode-bidi` with the Unicode 18 source, never a whole-text level vector sliced by paragraph) → L3 mark fixup → L4 mirroring with that paragraph's resolved levels → internal skeleton. No silent all-LTR mismatch fallback: a level/paragraph shape violation fails loudly |
| `are_confusable(a, b)` | True when two *distinct* strings share the exact skeleton |

Source mappings answer "has a confusable mapping"; skeleton equality answers
"has a confusable skeleton collision"; mixed-scripts answers "spoof mixture";
edit-distance answers "near match". These are four distinct verdicts — see
`identifier_inspect` / `identifier_table_inspect`, which report UTS #39
collisions as `confusable` and edit-distance proximity as `near_match`.
...
- **Rust**: Unicode XID rules via the `unicode-ident` crate (`XID_Start`/`XID_Continue` plus `_`) through the shared `rust_identifier_is_valid()` helper used by both `identifier_analyze()` and `identifier_inspect()` (keyword detection kept separate); `is_valid_rust_identifier_ascii()` retains the explicit ASCII-only verdict for stricter policy surfaces
...
- Per-identifier: normalization, scripts, invisible characters (central `has_security_invisible_hazard`), confusables, validity (Rust via the shared XID+keyword helper, not fallthrough)
- Mixed-script warnings via UTS #39 resolved sets (`is_mixed_script`), not the legacy allowance
- **Collisions**: UTS #39 `confusable` pairs (exact public-skeleton equality, one skeleton per normalized identifier), `near_match` pairs (edit distance ≤ 1 without skeleton equality), casefold collisions, normalization collisions
...
- `confusable`: UTS #39 skeleton collisions (`confusable`) plus edit-distance ≤1 proximity reported separately as `near_match`
...
## Temporal core (`src/temporal/`)

Two files, `pub(crate)` (not part of `src/text/`): `mod.rs` (fixed-offset datetime primitives) and `cron.rs` (5-field cron parser + next-occurrence search).

### Explicit-inputs contract

Deterministic utils take explicit inputs only — no clock, no TZ database, no env, no network:

- All timestamps are caller-supplied RFC 3339 strings (`parse_rfc3339` / `format_rfc3339`); all offsets are explicit (`parse_fixed_offset` accepts exactly `Z` or `±HH:MM`, hours ≤ 23, minutes ≤ 59).
- Computation preserves the input's fixed offset (`search_next` re-applies `after.offset()` to results); `date_components` / `unix_nanos` / `unix_unit` are pure functions of the given instant.
- Identical inputs and execution options give identical outputs within one eggsact version.
...
`config_file_inspect` (in `src/tools/repo.rs`, backed by `src/text/` config primitives) has **no YAML parser**: YAML analysis is heuristic-only — a naive line-oriented `key: value` scan (`yaml_key_values`). `parse_ok` is true when the scan completes on non-empty input; it must not be read as YAML syntax validity. The additive `analysis_mode` result field reports `"parser"` (real-parser verdict: JSON/TOML family) vs `"heuristic"` (line scan, no syntax validation: YAML, dotenv, INI, unknown). `config_preflight` excludes YAML for the same reason.

## Limits

Enforced early with descriptive errors; truncation is automatic — check `limits_applied` in tool responses.

| Input | Limit | Where |
|-------|-------|-------|
| Text | 100,000 chars (`MAX_TEXT_LENGTH`, incl. `MAX_INPUT_length`, `MAX_TEXT_LENGTH_REGEX`) | `validate.rs`, `replace.rs`, `line_range.rs`, `unicode_policy.rs`, `toml.rs` |
| Expression | 10,000 chars | `src/calc/` (not a text limit) |
| List items | 10,000 (`MAX_LIST_ITEMS`) | `validate.rs` (`list_dedupe`, `list_sort`) |
| Regex pattern | 1,000 chars, nesting depth 5 (`MAX_PATTERN_LENGTH`, `MAX_PATTERN_NESTING`) | `validate.rs`, `regex_safety.rs` |
| Regex samples | max 100 samples (`MAX_REGEX_SAMPLES`), each ≤ 10,000 chars (`MAX_SAMPLE_LENGTH`) | `src/tools/helpers.rs`, `src/tools/regex.rs`, `validate.rs` |
| Edit distance / diff | 10,000 chars per input; 4M-cell DP budget (`DIFF_SPANS_MAX_CELLS`) with coarse fallback | `diff.rs` |
| Request / output | 1,000,000 bytes each (`MAX_REQUEST_BYTES`, `MAX_OUTPUT_BYTES`) | `src/mcp/runtime.rs`, `src/mcp/budget.rs` |
| Prompt findings | > 1000 findings sorted by severity and truncated (`findings_truncated: true`) | `inspect_prompt.rs` |
