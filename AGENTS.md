...
- **Discovery evaluation is deterministic and offline:** `src/mcp/discovery_eval.rs` measures exact serialized Tool-definition bytes; `tests/fixtures/tool_discovery_intents.json` and `tests/fixtures/tool_discovery_scenarios.json` are the frozen retrieval/evaluation corpus; `scripts/score-discovery-traces.py` scores recorded provider/client traces without network access. Keep the retrieval gates at top-1 >=90%, top-3 >=98%, top-5 100%, discovery/full-Model <=25% of direct bytes, and zero Model-audience leaks. Provider-backed model traces are maintainer evidence, not ordinary CI.
