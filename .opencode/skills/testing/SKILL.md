...
- `EvalContext` is propagated through `math_eval` via thread-local (PRNG seeds, MCP mode restrictions); access via `budget::with_current_eval_context(|ctx| ...)` (closure-scoped)
