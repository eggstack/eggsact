...
- One stdio process is one connection pinned by its opening exchange (`ConnectionEra::Undecided` → `Legacy` on successful `initialize`, → `Modern20260728` on a valid modern envelope). Legacy uses `initialize` → `notifications/initialized` + `SessionState` (inside Legacy-pinned connections only). Modern validates `_meta` (`io.modelcontextprotocol/protocolVersion` + `clientCapabilities`, optional `clientInfo`) per request; `server/discover` works from a fresh process, unversioned `server/discover` never pins and never touches `SessionState`.
- Cross-era requests after pinning are rejected with `-32600`/`ERA_MISMATCH` (id preserved) via `era_mismatch()`; invalid envelopes (`-32602`/`-32022`) and failed `initialize` validation never pin. The era lock is held only for inspect/select — never across tool execution. Exactly one concurrent opening wins.
