...
|---|---|---|---|---|
| Deterministic tool substrate | active | `plans/subsystems/deterministic-tool-substrate-roadmap.md` | Guard only; no open milestone | Continuous invariant guard (registry sync, typed hierarchy, limits). |
...
- No milestone in substrate, harness, or distribution workstreams remains
  ready or blocked. Those lines are closed guards; reopen only via a new
  corrective implementation plan referencing the original archive record.
