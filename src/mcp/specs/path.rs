...
pub const PATH_TOOLS: &[ToolSpec] = &[
    ...
    ToolSpec {
        name: "path_batch_scope_check",
        description: "Check many target paths against a workspace root in one call. Detects escaping paths, absolute paths, traversal segments, and duplicate normalized targets.",
        handler: path_batch_scope_check,
        input_schema: path_batch_scope_check_input,
        output_schema: path_batch_scope_check_output,
        category: "path",
        tier: 2,
        profiles: &["full", "codegg_preflight", "codegg_patch"],
        tags: &["text", "path", "security", "scope", "batch"],
        exposure: ToolExposure::HarnessOnly,
        harness_use: &["path_preflight"],
        aliases: &[],
        cost: ToolCost::Cheap,
