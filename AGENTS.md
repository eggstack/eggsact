...
- **Runtime metrics**: `RUNTIME_METRICS` provides live atomic counters (`active_requests`, `active_blocking_handlers`, `timed_out_handlers`, `total_timeouts`, `peak_blocking_concurrency`). RAII `MetricGuard` ensures correct decrement on panic/unwind. `snapshot_metrics()` returns a point-in-time snapshot.
- **Nested thread lifetime**: `run_with_timeout` in `src/tools/helpers.rs` spawns `std::thread::spawn` with its own `recv_timeout`. This inner thread can outlive the handler by 5–30 seconds after the outer tokio timeout fires. It does not participate in cooperative cancellation. Affected: `math_eval`, `validate_regex`, `regex_finditer`, `ini_validate`.
