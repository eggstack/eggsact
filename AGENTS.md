...
- **Adding an MCP tool requires one `ToolSpec` entry** in `src/mcp/registry/all_tools.rs`. This is the single source of truth for tool registration. A test (`tool_registration_tables_are_in_sync`) will catch drift.
